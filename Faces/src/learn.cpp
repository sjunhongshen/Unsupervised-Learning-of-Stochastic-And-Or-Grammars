														 ...
														 VertexId sample_root, int n_samples, int n_gibbs)
{
	...
	for(int i = 0; i < n_samples; i++)
	{
		...
		for(auto re: res)
		{
			...
			// if(aog.GetVertexContent(re.first)->GetState().GetContent().compare(0, 3, "Fac") == 0)
			// 	cout << configurations.back().at(re)[3];
		}
...
AOG<string, double> CreateFaceAOG()
{
	...
	for(auto leaf_state: leaf_states)
    {
		...
		aog.SetUnaryPotentialGradFunc(aog.GetVertexIdByState(leaf_state), LeafPotentialGradFunc);
		VertexId vid = aog.GetVertexIdByState(leaf_state);
		if (leaf_state.GetContent()[0] == 'F')
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, face_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(face_prop);
		}
		else if (leaf_state.GetContent()[0] == 'N')
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, nose_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(nose_prop);
		}
		else if (leaf_state.GetContent()[0] == 'M')
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, mouth_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(mouth_prop);
		}
		else if (leaf_state.GetContent().compare(0, 3, "LEa") == 0)
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, ear_left_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(ear_left_prop);
		}
		else if (leaf_state.GetContent().compare(0, 3, "REa") == 0)
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, ear_right_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(ear_right_prop);
		}
		else if (leaf_state.GetContent().compare(0, 3, "LEy") == 0)
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, eye_left_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(eye_left_prop);
		}
		else if (leaf_state.GetContent().compare(0, 3, "REy") == 0)
		{
			aog.SetVertexAttributesRangesFunc(vid, AttributesRangesFunc, eye_right_prop);
			aog.GetVertexContent(vid)->UpdateAttributesRanges();
			aog.GetVertexContent(vid)->SetPotentialParam(eye_right_prop);
		}
	}
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(leftEye), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(rightEye), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(leftEar), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(rightEar), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(nose), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
	...
	aog.SetBinaryPotentialGradFunc(make_pair(aog.GetVertexIdByState(mouth), aog.GetVertexIdByState(faceShape)), RelativeToFacePotentialGradFunc);
...
int main(int argc, char *argv[])
{
	...
	std::ifstream ifs(directory + "/face_details");
    boost::archive::binary_iarchive ia(ifs);
    vector<pair<unordered_map<pair<Symbolic_State<string>, int>, vector<double>, AOG_LIB::pair_hash>,
				unordered_map<pair<Symbolic_State<string>, int>, vector<pair<Symbolic_State<string>, int> >, AOG_LIB::pair_hash> > > sampled_data;
	ia >> sampled_data;
	vector<double> lrs(param_reference.size(), 1e-1);
	Learn(faceAOG, sampled_data, param_reference, lrs, 300);

    faceAOG.Visualize("./", "./cartoon_face_grammar_learn_vis.txt");
	faceAOG.SaveGraph("./", "cartoon_face_grammar_learn.txt");

    directory += "/Learned";

	printf("Sampling %s %s samples at \\%s\n", args["-n"].c_str(), 
		false ? "negative" : "positive", directory.c_str());
	...
	DrawFaces(directory, faces, false);
	return 0;
}